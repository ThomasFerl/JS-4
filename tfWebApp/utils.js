...
export function webApiRequest_based_on_GET( _cmd , _param  )
{
  ...
  { 
    ...
    {
      dialogs.showMessage('<B>S</B>itzung abgelaufen !' , {glyp:"fa-solid fa-person-walking-arrow-right",button:['OK']})
...
export function POSTrequest(_cmd, _param, downloadURL) 
{
  ...
  fetch(globals.URL_webAppPOSTrequest(), { method: 'POST', headers: {'Content-Type': 'application/json'}, body: requestBody} )
      ...
      .catch(error => {
                        ...
                        dialogs.showMessage(error.message);
                     });
}
...
export function _copyStringToClipboard (str) 
{
  ...
  navigator.clipboard
    .writeText(str)
    .then(() => {
      dialogs.showMessage("Inhalt in Zwischenablage kopiert");
    })
    .catch(() => {
      dialogs.showMessage("Inhalt konnte nicht in die Zwischenablage kopiert werden !");
    });
}
